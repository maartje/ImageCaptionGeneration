class LossCollector():
    ...
    def process_train_loss(self, epoch, batch_index, token_loss, epoch_finished):
        self.process_tmp_loss(epoch, batch_index, token_loss, epoch_finished)
        self.process_batch_loss(epoch, batch_index, token_loss, epoch_finished)
        self.process_epoch_loss(epoch, batch_index, token_loss, epoch_finished)

    # Step 1: add loss to _tmp_losses, clear _tmp_losses when new epoch has started
    def process_tmp_loss(self, epoch, batch_index, token_loss, epoch_finished):
        if batch_index == 0:        
            self._tmp_losses = []
        self._tmp_losses.append(token_loss)

    # Step 2: store batch loss in batch losses, when batch interval
    def process_batch_loss(self, epoch, batch_index, token_loss, epoch_finished):
...
#        print('store partial batch', epoch, batch_index, losses_partial_batch)
    ...
    def process_epoch_loss(self, epoch, batch_index, token_loss, epoch_finished):
        if epoch_finished:        
